# modules/hq_admin/analytics_tab.py
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
# 본사관리자 — 폐기물 발생 분석 대시보드
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
import streamlit as st
import pandas as pd
from datetime import datetime

from database.db_manager import db_get
from services.waste_analytics import (
    summary_kpis, daily_stats, weekday_stats, monthly_stats,
    seasonal_stats, school_period_stats,
    by_school_stats, by_item_stats, by_driver_stats,
    trend_analysis, detect_anomalies, weather_correlation,
)


def render_analytics_tab():
    """본사관리자 폐기물 발생 분석 화면"""
    st.markdown("## 📊 폐기물 발생 분석")

    # ── 데이터 로드 ──
    all_rows = db_get('real_collection') or []
    if not all_rows:
        st.info("수거 데이터가 없습니다. 수거 데이터가 등록되면 분석이 표시됩니다.")
        return

    # ── 필터 영역 ──
    col_f1, col_f2, col_f3 = st.columns(3)

    # 연도/월 옵션 생성
    dates = [str(r.get('collect_date', ''))[:7] for r in all_rows if r.get('collect_date')]
    ym_set = sorted(set(d for d in dates if len(d) == 7), reverse=True)
    years = sorted(set(d[:4] for d in ym_set), reverse=True)

    with col_f1:
        sel_year = st.selectbox("연도", ['전체'] + years, key='hq_ana_year')
    with col_f2:
        if sel_year != '전체':
            months_in_year = [ym for ym in ym_set if ym.startswith(sel_year)]
            sel_ym = st.selectbox("월", ['전체'] + months_in_year, key='hq_ana_ym')
        else:
            sel_ym = st.selectbox("월", ['전체'] + ym_set, key='hq_ana_ym')
    with col_f3:
        vendors = sorted(set(str(r.get('vendor', '')) for r in all_rows if r.get('vendor')))
        sel_vendor = st.selectbox("업체", ['전체'] + vendors, key='hq_ana_vendor')

    # 필터 적용
    filtered = all_rows
    if sel_vendor != '전체':
        filtered = [r for r in filtered if r.get('vendor') == sel_vendor]

    year_val = int(sel_year) if sel_year != '전체' else None
    ym_val = sel_ym if sel_ym != '전체' else None

    # ── 탭 구성 ──
    tab1, tab2, tab3, tab4, tab5 = st.tabs([
        "📈 종합 현황", "📅 일별·요일별", "📊 월별·계절별",
        "🏫 거래처·기사별", "🌤️ 기상 상관분석"
    ])

    # ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
    # TAB 1: 종합 현황
    # ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
    with tab1:
        kpi = summary_kpis(filtered, year_month=ym_val)
        if not kpi:
            st.info("선택한 기간의 데이터가 없습니다.")
        else:
            # KPI 카드
            c1, c2, c3, c4 = st.columns(4)
            with c1:
                delta_str = f"{kpi['mom_change_pct']:+.1f}%" if kpi.get('mom_change_pct') is not None else None
                st.metric("총 수거량", f"{kpi['total_kg']:,.1f} kg", delta=delta_str)
            with c2:
                st.metric("일평균 수거량", f"{kpi['avg_daily_kg']:,.1f} kg/일")
            with c3:
                st.metric("수거일수", f"{kpi['collection_days']}일")
            with c4:
                st.metric("거래처 수", f"{kpi['school_count']}개소")

            c5, c6, c7, c8 = st.columns(4)
            with c5:
                st.metric("음식물", f"{kpi['food_kg']:,.1f} kg")
            with c6:
                st.metric("재활용", f"{kpi['recycle_kg']:,.1f} kg")
            with c7:
                st.metric("일반", f"{kpi['general_kg']:,.1f} kg")
            with c8:
                st.metric("Top 거래처", kpi['top_school'],
                          delta=f"{kpi['top_school_kg']:,.1f} kg")

            st.divider()

            # 품목별 파이차트 + 추세
            col_pie, col_trend = st.columns(2)
            with col_pie:
                st.markdown("#### 품목별 비율")
                item_df = by_item_stats(filtered, year_month=ym_val)
                if not item_df.empty:
                    st.bar_chart(
                        item_df.set_index('item_type')['total_kg'],
                        use_container_width=True
                    )
                else:
                    st.info("데이터 없음")

            with col_trend:
                st.markdown("#### 월별 추세")
                t_df = trend_analysis(filtered, period='monthly')
                if not t_df.empty:
                    st.line_chart(
                        t_df.set_index('period')['total_kg'],
                        use_container_width=True
                    )
                else:
                    st.info("데이터 없음")

            # 이상치 알림
            st.divider()
            st.markdown("#### ⚠️ 이상치 탐지")
            anomaly_df = detect_anomalies(filtered, year_month=ym_val)
            if not anomaly_df.empty:
                anomalies = anomaly_df[anomaly_df['is_anomaly'] == True]
                if len(anomalies) > 0:
                    st.warning(f"이상 수거량이 감지된 날이 {len(anomalies)}일 있습니다.")
                    st.dataframe(
                        anomalies[['collect_date', 'total_kg', 'z_score']].rename(columns={
                            'collect_date': '날짜', 'total_kg': '수거량(kg)', 'z_score': 'Z-Score'
                        }),
                        use_container_width=True, hide_index=True
                    )
                else:
                    st.success("이상치가 감지되지 않았습니다.")
            else:
                st.info("분석할 데이터가 부족합니다.")

    # ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
    # TAB 2: 일별·요일별
    # ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
    with tab2:
        st.markdown("### 📅 일별 수거량")
        d_df = daily_stats(filtered, year_month=ym_val)
        if not d_df.empty:
            d_df['collect_date'] = pd.to_datetime(d_df['collect_date'])
            chart_df = d_df.set_index('collect_date')[['food_kg', 'recycle_kg', 'general_kg']]
            chart_df.columns = ['음식물', '재활용', '일반']
            st.area_chart(chart_df, use_container_width=True)

            with st.expander("일별 상세 데이터", expanded=False):
                show_df = d_df.copy()
                show_df['collect_date'] = show_df['collect_date'].dt.strftime('%Y-%m-%d')
                st.dataframe(
                    show_df.rename(columns={
                        'collect_date': '날짜', 'total_kg': '합계(kg)',
                        'food_kg': '음식물', 'recycle_kg': '재활용',
                        'general_kg': '일반', 'count': '건수'
                    }),
                    use_container_width=True, hide_index=True
                )
        else:
            st.info("선택한 기간의 일별 데이터가 없습니다.")

        st.divider()

        st.markdown("### 📊 요일별 평균 수거량")
        w_df = weekday_stats(filtered, year_month=ym_val)
        if not w_df.empty:
            w_chart = w_df.set_index('weekday_name')['avg_kg']
            st.bar_chart(w_chart, use_container_width=True)

            st.caption("요일별 상세")
            st.dataframe(
                w_df[['weekday_name', 'avg_kg', 'total_kg', 'count']].rename(columns={
                    'weekday_name': '요일', 'avg_kg': '평균(kg)',
                    'total_kg': '합계(kg)', 'count': '건수'
                }),
                use_container_width=True, hide_index=True
            )
        else:
            st.info("요일별 데이터가 없습니다.")

    # ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
    # TAB 3: 월별·계절별
    # ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
    with tab3:
        st.markdown("### 📊 월별 수거량")
        m_df = monthly_stats(filtered, year=year_val)
        if not m_df.empty:
            m_chart = m_df.set_index('ym')[['food_kg', 'recycle_kg', 'general_kg']]
            m_chart.columns = ['음식물', '재활용', '일반']
            st.bar_chart(m_chart, use_container_width=True)

            # 증감률 표시
            t_df = trend_analysis(filtered, period='monthly')
            if not t_df.empty and 'change_pct' in t_df.columns:
                st.markdown("#### 전월 대비 증감률")
                change_df = t_df.dropna(subset=['change_pct'])
                if not change_df.empty:
                    st.line_chart(
                        change_df.set_index('period')['change_pct'],
                        use_container_width=True
                    )
        else:
            st.info("월별 데이터가 없습니다.")

        st.divider()

        col_s1, col_s2 = st.columns(2)
        with col_s1:
            st.markdown("### 🌸 계절별 수거량")
            s_df = seasonal_stats(filtered)
            if not s_df.empty:
                st.bar_chart(
                    s_df.set_index('season')['avg_daily_kg'],
                    use_container_width=True
                )
                st.dataframe(
                    s_df[['season', 'total_kg', 'avg_daily_kg', 'count']].rename(columns={
                        'season': '계절', 'total_kg': '합계(kg)',
                        'avg_daily_kg': '일평균(kg)', 'count': '건수'
                    }),
                    use_container_width=True, hide_index=True
                )
            else:
                st.info("데이터 없음")

        with col_s2:
            st.markdown("### 🎓 학사일정별 수거량")
            p_df = school_period_stats(filtered)
            if not p_df.empty:
                st.bar_chart(
                    p_df.set_index('period')['avg_daily_kg'],
                    use_container_width=True
                )
                st.dataframe(
                    p_df[['period', 'total_kg', 'avg_daily_kg', 'count']].rename(columns={
                        'period': '기간', 'total_kg': '합계(kg)',
                        'avg_daily_kg': '일평균(kg)', 'count': '건수'
                    }),
                    use_container_width=True, hide_index=True
                )
            else:
                st.info("데이터 없음")

    # ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
    # TAB 4: 거래처·기사별
    # ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
    with tab4:
        st.markdown("### 🏫 거래처별 수거량 TOP 20")
        sch_df = by_school_stats(filtered, year_month=ym_val, top_n=20)
        if not sch_df.empty:
            st.bar_chart(
                sch_df.set_index('school_name')[['food_kg', 'recycle_kg']],
                use_container_width=True
            )
            st.dataframe(
                sch_df.rename(columns={
                    'school_name': '거래처', 'total_kg': '합계(kg)',
                    'food_kg': '음식물(kg)', 'recycle_kg': '재활용(kg)',
                    'count': '건수'
                }),
                use_container_width=True, hide_index=True
            )
        else:
            st.info("거래처 데이터가 없습니다.")

        st.divider()

        st.markdown("### 🚛 기사별 수거 실적")
        drv_df = by_driver_stats(filtered, year_month=ym_val)
        if not drv_df.empty:
            st.bar_chart(
                drv_df.set_index('driver')['total_kg'],
                use_container_width=True
            )
            st.dataframe(
                drv_df.rename(columns={
                    'driver': '기사', 'total_kg': '합계(kg)',
                    'count': '건수', 'schools': '담당 거래처수'
                }),
                use_container_width=True, hide_index=True
            )
        else:
            st.info("기사 데이터가 없습니다.")

    # ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
    # TAB 5: 기상 상관분석
    # ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
    with tab5:
        st.markdown("### 🌤️ 기상 데이터 × 수거량 상관분석")
        st.caption("기상청 API 키가 설정되어 있어야 날씨 데이터를 조회할 수 있습니다.")

        # 분석 기간 설정
        col_w1, col_w2 = st.columns(2)
        with col_w1:
            w_start = st.date_input("시작일", key='hq_w_start',
                                     value=datetime(datetime.now().year, datetime.now().month, 1))
        with col_w2:
            w_end = st.date_input("종료일", key='hq_w_end',
                                   value=datetime.now())

        if st.button("기상 상관분석 실행", key='hq_run_weather'):
            with st.spinner("기상 데이터 조회 중..."):
                try:
                    from services.weather_api import fetch_daily_weather
                    weather_result = fetch_daily_weather(
                        w_start.strftime('%Y-%m-%d'),
                        w_end.strftime('%Y-%m-%d')
                    )
                except ImportError:
                    weather_result = {'success': False, 'message': 'weather_api 모듈 없음', 'data': []}

                if not weather_result.get('success'):
                    st.error(f"기상 데이터 조회 실패: {weather_result.get('message', '알 수 없는 오류')}")
                else:
                    st.success(weather_result.get('message', ''))

                    # 기간에 해당하는 수거 데이터
                    start_str = w_start.strftime('%Y-%m-%d')
                    end_str = w_end.strftime('%Y-%m-%d')
                    period_rows = [r for r in filtered
                                   if start_str <= str(r.get('collect_date', ''))[:10] <= end_str]

                    result = weather_correlation(period_rows, weather_result['data'])

                    if result is None:
                        st.warning("병합할 수 있는 데이터가 없습니다. 수거 데이터와 날씨 데이터의 날짜가 겹치는지 확인하세요.")
                    else:
                        # 상관계수 표시
                        st.markdown("#### 상관계수 (수거량 vs 기상요소)")
                        corr = result['correlations']
                        LABELS = {'temp_avg': '평균기온', 'rain': '강수량',
                                  'humidity': '습도', 'wind': '풍속'}
                        cc1, cc2, cc3, cc4 = st.columns(4)
                        cols_map = [cc1, cc2, cc3, cc4]
                        for idx, (k, v) in enumerate(corr.items()):
                            with cols_map[idx % 4]:
                                color = "🔴" if abs(v) > 0.5 else "🟡" if abs(v) > 0.3 else "🟢"
                                st.metric(f"{color} {LABELS.get(k, k)}", f"{v:+.3f}")

                        st.divider()

                        # 비 오는 날 vs 맑은 날
                        st.markdown("#### 비 오는 날 vs 맑은 날")
                        rv = result['rainy_vs_clear']
                        rc1, rc2, rc3 = st.columns(3)
                        with rc1:
                            st.metric("비 오는 날 평균", f"{rv['rainy_avg']:,.1f} kg")
                        with rc2:
                            st.metric("맑은 날 평균", f"{rv['clear_avg']:,.1f} kg")
                        with rc3:
                            st.metric("차이", f"{rv['diff_pct']:+.1f}%")

                        # 기온 구간별
                        if not result['temp_bins'].empty:
                            st.markdown("#### 기온 구간별 평균 수거량")
                            st.bar_chart(
                                result['temp_bins'].set_index('temp_range')['avg_kg'],
                                use_container_width=True
                            )

                        # 병합 데이터 표시
                        with st.expander("병합 데이터 상세", expanded=False):
                            show_cols = ['date_key', 'total_kg', 'temp_avg', 'temp_max',
                                         'temp_min', 'rain', 'humidity', 'wind']
                            avail_cols = [c for c in show_cols if c in result['merged_df'].columns]
                            st.dataframe(
                                result['merged_df'][avail_cols].rename(columns={
                                    'date_key': '날짜', 'total_kg': '수거량(kg)',
                                    'temp_avg': '평균기온', 'temp_max': '최고기온',
                                    'temp_min': '최저기온', 'rain': '강수량(mm)',
                                    'humidity': '습도(%)', 'wind': '풍속(m/s)'
                                }),
                                use_container_width=True, hide_index=True
                            )
