...
### 0.3 코워크 VM 마운트 파일 크기 한계 (2026-04-13 발견)

> ⚠️ **코워크 VM이 Windows 폴더를 마운트할 때 약 190KB 이상 파일의 끝부분이 잘려서 읽힙니다.**

**영향받는 파일 (2026-04-13 기준)**

| 파일 | Windows 실제 | 코워크 표시 | 누락 내용 |
|---|---|---|---|
| `pages/hq_admin.py` | 4,630줄 | ~4,433줄 | `_photo_tab()` 중간 + `hq_admin_page()` |
| `pages/vendor_admin.py` | 4,654줄 | ~4,511줄 | `_analytics_tab()` + `vendor_admin_page()` |
| `pages/driver.py` | 2,085줄 | ~2,072줄 | `driver_page()` 마지막 부분 |
| `pages/school.py` | 847줄 | ~829줄 | `school_page()` 본문 |

**핵심 규칙**
1. **코워크는 위 파일을 직접 Edit 하지 않는다** — Edit 도구가 잘린 내용만 덮어쓰면 파일이 손상됨
2. **위 파일은 반드시 작업지시서 방식** → 디스패치에서 Windows 네이티브 환경에서 수정
3. **작은 파일 (~100KB 이하)은 코워크 직접 편집 가능** — shared.py, colors.py, login.py 등
4. **파일 크기가 커지면 이 목록에 추가할 것** — `wc -c`로 190,000바이트 이상이면 위험
...
### ✅ UI/UX 상용화 Phase 1~5 (2026-04-13)

| Phase | 내용 | 상태 |
|---|---|---|
| UI-1 | 디자인 시스템 통일 (shared.py 공통 컴포넌트) | ✅ 완료 |
| UI-2 | 컬러/타이포 통일 (colors.py + 전 페이지 적용) | ✅ 완료 |
| UI-3 | 관리자 모바일 사이드바 (햄버거 드로어) | ✅ 완료 |
| UI-4 | 데이터 테이블 모바일 카드 레이아웃 | ✅ 완료 |
| UI-5 | 토스트 알림 통일 기반 (헬퍼함수 + 정책문서) | ✅ 완료 |

**UI-5 산출물:**
- `components/shared.py` — `notify_success/error/warning/info` 헬퍼 함수 + `TOAST_DEFAULTS` 설정
- `zeroda_reflex/NOTIFICATION_POLICY.md` — 알림 정책 문서 (토스트 vs 인라인 콜아웃 기준)
- 신규 코드: 반드시 `notify_*` 헬퍼 사용, `rx.toast` 직접 호출 금지
- 기존 코드 전환: 상용화 2~3단계에서 점진적 마이그레이션
...
### 사례 8: 코워크 VM 마운트 파일 잘림 (2026-04-13)

**사고 경위**
- UI Phase 4 작업 중 코워크가 vendor_admin.py, hq_admin.py를 직접 Edit으로 수정
- 수정 자체는 정상이었으나, 코워크에서 파일 끝이 잘려보이는 것을 발견
- 조사 결과 **코워크 VM 마운트가 약 190KB 이상 파일의 끝부분을 잘라서 읽음**
- `vendor_admin_page()`, `hq_admin_page()` 등 핵심 함수가 코워크에 보이지 않았음
- 잘린 파일을 그대로 git push하면 서버 다운 위험

**영향 범위**
- hq_admin.py (4,630줄 중 ~197줄 미표시), vendor_admin.py (4,654줄 중 ~143줄 미표시)
- driver.py (~13줄 미표시), school.py (~18줄 미표시)
- edu_office.py, meal_manager.py, login.py 등 100KB 이하 파일은 정상

**복구 과정**
1. `git checkout origin/main -- <파일>` 으로 4개 파일 GitHub에서 복원
2. Phase 4 수정사항은 작업지시서로 전환, 디스패치에서 재적용

**교훈**
1. **코워크는 190KB 이상 파일을 직접 Edit 하지 않는다** — 섹션 0.3에 규칙으로 명시
2. **큰 파일 수정은 반드시 작업지시서 방식** → 디스패치가 Windows 네이티브에서 수정
3. **작은 파일(~100KB 이하)은 코워크 직접 편집 가능** — shared.py, colors.py 등
4. **파일 크기 사전 확인** — `wc -c`로 190,000바이트 이상이면 작업지시서 방식으로 전환
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  